class_name AttractionSelectBTN extends Button;
...
@export var attraction_data: AttractionData;
...
func _ready() -> void:
	...
	sprite.set_texture(attraction_data.texture);
	price.set_text("$%d" % attraction_data.cost_to_build);
...
func _on_park_data_money_changed(new_money: int) -> void:
	if new_money < attraction_data.cost_to_build:
		lock();
	else:
		unlock();
